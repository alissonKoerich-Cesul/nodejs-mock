...
import { generateJwtAndfreshToken } from "./auth";
...
function checkAuthMiddleware(request: Request, response: Response, next: NextFunction){
    ...
    if(!authorization){
        return response.status(414).json({error:true , code: 'token.invalid', message:'Token not exists'});
...
app.post("/sessions",(request,response) =>{
    ...
    const {token } = generateJwtAndfreshToken(email,{
